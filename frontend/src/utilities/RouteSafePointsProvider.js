...
export function filterCrimeData(origin, dest, weight_limit, crimeData) {
  ...
  let minLat = Math.min(...crimeData.map((row) => parseFloat(row.location.lat())));
  let maxLat = Math.max(...crimeData.map((row) => parseFloat(row.location.lat())));
  let minLng = Math.min(...crimeData.map((row) => parseFloat(row.location.lng())));
  let maxLng = Math.max(...crimeData.map((row) => parseFloat(row.location.lng())));
